...
class EvolutionCard {
    ...
    public function checkCanPlay(Context $context) {
        if ($this->mindbugKeywords !== null) {
            throw new UserException(clienttranslate("You can only play this evolution card by playing the associated keyword"));
        }

        if ($context->game->EVOLUTION_CARDS_TYPES[$this->type] == 3) {
            throw new UserException(clienttranslate("You can only play this evolution now, you'll be asked to use it when you wound a Monster"));
        }
        ...
        switch($this->type) {
            ...
            case RABBIT_S_FOOT_EVOLUTION:
                throw new UserException(clienttranslate("You can't play this Evolution now, you'll be asked to use it when you'll take damage"));
            ...
            case FELINE_MOTOR_EVOLUTION:
                ...
                if (in_array($context->currentPlayerId, $startedTurnInTokyo)) {
                    throw new UserException(clienttranslate("You started your turn in Tokyo"));
                }
                break;
            ...
            case EATS_SHOOTS_AND_LEAVES_EVOLUTION:
                if (!$context->game->inTokyo($context->currentPlayerId)) {
                    throw new UserException(clienttranslate("You can play this Evolution only if you are in Tokyo"));
                }
                break;
            case JUNGLE_FRENZY_EVOLUTION:
                if ($context->currentPlayerId != intval($context->game->getActivePlayerId())) {
                    throw new UserException(clienttranslate("You must play this Evolution during your turn"));
                }
                if ($context->game->inTokyo($context->currentPlayerId)) {
                    throw new UserException(clienttranslate("You can play this Evolution only if you are not in Tokyo"));
                }
                if (!$context->game->isDamageDealtThisTurn($context->currentPlayerId)) {
                    throw new UserException(clienttranslate("You didn't deal damage to a player in Tokyo"));
                }
                break;
            ...
            case ICY_REFLECTION_EVOLUTION:
                ...
                if (!$canPlayIcyReflection) {
                    throw new UserException(clienttranslate("You can only play this evolution card when there is another permanent Evolution on the table"));
                }
                break;
        }
    }
